...
public class TestDrawPass {
    ...
    public static void main(String[] args) {
        ...
        ImmediateContext immediateContext = makeOpenGL(
        ...
        for (int i = 0; i < nRects; i++) {
            ...
            transform.preTranslate(-cx, -cy);
            Draw draw = new Draw(new Matrix(transform), rrect, false);
        ...
        GraphicsPipelineDesc graphicsPipelineDesc = new GraphicsPipelineDesc(step, Key.EMPTY, true);

        int ubo = glGenBuffers();
        glBindBufferBase(GL_UNIFORM_BUFFER, 0, ubo);
        GL44C.glBufferStorage(GL_UNIFORM_BUFFER, new float[]{2.0f / 1280, -1.0f, -2.0f / 720, 1.0f}, 0);
        LOGGER.info("UBO: {}", ubo);

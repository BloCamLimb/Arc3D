...
// We'll round up to 300 to keep it simple.
    ...
    public static float divide_and_pin(float numer, float denom, float min, float max) {
    ...
    public static void drawShadow(Canvas canvas,
        ...
        if (Color.alpha(ambientColor) > 0) {
            ...
            RRect r2 = new RRect();
            r2.setRectXY(rrect.left()-strokeWidth, rrect.top()-strokeWidth,
                    rrect.right()+strokeWidth, rrect.bottom()+strokeWidth,
        ...
        if (Color.alpha(spotColor) > 0) {
            float occluderZ = rrect.centerX() * zPlane0 + rrect.centerY() * zPlane1 + zPlane2;

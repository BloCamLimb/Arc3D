...
public class TestCompiler {
    ...
    public static void main(String[] args) {
        ...
        {
            long bytes = 0;
            bytes += 16 + (Lexer.MAPPINGS.length);
            bytes += 16 + (Lexer.ACCEPTS.length);
            bytes += 16 + (Lexer.INDICES.length * 2);
            bytes += 16 + (Lexer.FULL.length * 4);
            for (short[] elem : Lexer.FULL) {
                bytes += 16 + (elem.length * 2);
            }
            bytes += 16 + (Lexer.PACKED.length * 4);
            for (Lexer.PackedEntry elem : Lexer.PACKED) {
                bytes += 16 + 4 + 4 + 16 + (elem.data().length);
            }
        ...
        if (spirv != null) {
            ...
            testCompileToGLSL(spirv.rewind().asIntBuffer());
        }
    }

...
public final class ObjectIterators {
	 ...
	 */
	public static <K> int unwrap(final Iterator<K> i, final Collection<? super K> c, final int max) {
		if (max < 0) throw new IllegalArgumentException("The maximum number of elements (" + max + ") is negative");
		int j = max;
	 ...
	 *         collection (because of duplicates).
	 */
	public static <K> long unwrap(final Iterator<K> i, final Collection<? super K> c) {
		long n = 0;
		while (i.hasNext()) {
			c.add(i.next());
			n++;

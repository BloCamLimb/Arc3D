 ...
 * @see HashCommon
 */
public class ObjectObjectOpenHashMap<K, V> extends AbstractMap<K, V> implements ObjectObjectMap<K, V>, java.io.Serializable, Cloneable, Hash {
	...
	public ObjectObjectOpenHashMap(final int expected, final float f) {
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public ObjectObjectOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public ObjectObjectOpenHashMap(final Map<? extends K, ? extends V> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public ObjectObjectOpenHashMap(final Map<? extends K, ? extends V> m) {
	 ...
	 * @param f the load factor.
	 */
	public ObjectObjectOpenHashMap(final ObjectObjectMap<K, V> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public ObjectObjectOpenHashMap(final ObjectObjectMap<K, V> m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 */
	public ObjectObjectOpenHashMap(final K[] k, final V[] v, final float f) {
		this(k.length, f);
	 ...
	 * @throws IllegalArgumentException if {@code k} and {@code v} have different lengths.
	 */
	public ObjectObjectOpenHashMap(final K[] k, final V[] v) {
	...
	/** {@inheritDoc} */
	@Override
	public V computeIfAbsent(final K key, final ObjectObjectMap<? super K, ? extends V> mappingFunction) {
	...
	public void forEach(final java.util.function.BiConsumer<? super K, ? super V> consumer) {
		...
		final K key[] = ObjectObjectOpenHashMap.this.key;
	...
	private abstract class MapIterator<ConsumerType> {
		...
		boolean mustReturnNullKey = ObjectObjectOpenHashMap.this.containsNullKey;
		/**
		...
		private void shiftKeys(int pos) {
			...
			K curr;
			final K[] key = ObjectObjectOpenHashMap.this.key;
			final V value[] = ObjectObjectOpenHashMap.this.value;
			for (;;) {
				pos = ((last = pos) + 1) & mask;
				for (;;) {
					if (((curr = key[pos]) == null)) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				ObjectObjectOpenHashMap.this.remove(wrapped.set(-pos - 1, null));
	...
	private final class EntryIterator extends MapIterator<Consumer<? super ObjectObjectMap.Entry<K, V>>> implements java.util.Iterator<Entry<K, V>> {
		...
		@Override
		final void acceptOnIndex(final Consumer<? super ObjectObjectMap.Entry<K, V>> action, final int index) {
	...
	private final class FastEntryIterator extends MapIterator<Consumer<? super ObjectObjectMap.Entry<K, V>>> implements java.util.Iterator<Entry<K, V>> {
	...
	private final class MapEntrySet extends AbstractSet<Entry<K, V>> implements FastEntrySet<K, V> {
		@Override
		public Iterator<ObjectObjectMap.Entry<K, V>> iterator() {
			return new EntryIterator();
		}

		@Override
		public Iterator<ObjectObjectMap.Entry<K, V>> fastIterator() {
		...
		public boolean contains(final Object o) {
			...
			if (((k) == null)) return ObjectObjectOpenHashMap.this.containsNullKey && java.util.Objects.equals(value[n], v);
			K curr;
			final K[] key = ObjectObjectOpenHashMap.this.key;
			int pos;
			// The starting point.
		...
		/** {@inheritDoc} */
		@Override
		public void forEach(final Consumer<? super ObjectObjectMap.Entry<K, V>> consumer) {
		...
		/** {@inheritDoc} */
		@Override
		public void fastForEach(final Consumer<? super ObjectObjectMap.Entry<K, V>> consumer) {
	...
	@Override
	public FastEntrySet<K, V> entrySet() {
	 ...
	 */
	private final class KeyIterator extends MapIterator<Consumer<? super K>> implements java.util.Iterator<K> {
		public KeyIterator() {
			super();
		}
	...
	private final class KeySet extends AbstractSet<K> {
		...
		public boolean remove(Object k) {
			...
			ObjectObjectOpenHashMap.this.remove(k);
			return size != oldSize;
		}
	 ...
	 */
	private final class ValueIterator extends MapIterator<Consumer<? super V>> implements java.util.Iterator<V> {
	...
	@Override
	@SuppressWarnings("unchecked")
	public ObjectObjectOpenHashMap<K, V> clone() {
		ObjectObjectOpenHashMap<K, V> c;
		try {
			c = (ObjectObjectOpenHashMap<K, V>)super.clone();

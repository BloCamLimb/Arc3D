 ...
 * @see Map
 */
public interface ObjectIntMap<K> extends ToIntFunction<K> {
	/**
	 ...
	 */
	interface FastEntrySet<K> extends Set<Entry<K>> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super ObjectIntMap.Entry<K>> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @see Map#entrySet()
	 */
	Set<ObjectIntMap.Entry<K>> object2IntEntrySet();
	 ...
	 */
	default void forEach(final ObjectIntBiConsumer<? super K> consumer) {
		final Set<ObjectIntMap.Entry<K>> entrySet = object2IntEntrySet();
		final Consumer<ObjectIntMap.Entry<K>> wrappingConsumer = (entry) -> consumer.accept(entry.getKey(), entry.getIntValue());
    ...
    default void putAll(ObjectIntMap<? extends K> m) {
        Iterator<ObjectIntMap.Entry<K>> i;
        if (m.object2IntEntrySet() instanceof ObjectIntMap.FastEntrySet fastEntrySet) {
            i = fastEntrySet.fastIterator();
        } else {
            i = ((ObjectIntMap<K>) m).object2IntEntrySet().iterator();
        }
        while (i.hasNext()) {
            final ObjectIntMap.Entry<? extends K> e = i.next();
	 ...
	 * @since 8.0.0
	 */
	default int computeIfAbsent(final K key, final java.util.function.ToIntFunction<? super K> mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default int computeIfAbsent(final K key, final ObjectIntMap<? super K> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final int v = getInt(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.0.0
	 */
	default int computeIntIfPresent(final K key, final java.util.function.BiFunction<? super K, ? super Integer, ? extends Integer> remappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default int computeInt(final K key, final java.util.function.BiFunction<? super K, ? super Integer, ? extends Integer> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int oldValue = getInt(key), drv = defaultReturnValue();
	 ...
	 * @since 8.0.0
	 */
	default int merge(final K key, final int value, final java.util.function.BiFunction<? super Integer, ? super Integer, ? extends Integer> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
	 ...
	 * @since 8.5.0
	 */
	default int mergeInt(final K key, final int value, final java.util.function.IntBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int oldValue = getInt(key), drv = defaultReturnValue();

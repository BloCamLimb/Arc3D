 ...
 * @see HashCommon
 */
public class ObjectIntOpenHashMap<K> implements ObjectIntMap<K>, java.io.Serializable, Cloneable, Hash {
	...
	public ObjectIntOpenHashMap(final int expected, final float f) {
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public ObjectIntOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public ObjectIntOpenHashMap(final ObjectIntMap<K> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public ObjectIntOpenHashMap(final ObjectIntMap<K> m) {
	 ...
	 */
	public ObjectIntOpenHashMap(final K[] k, final int[] v, final float f) {
		this(k.length, f);
	 ...
	 * @throws IllegalArgumentException if {@code k} and {@code v} have different lengths.
	 */
	public ObjectIntOpenHashMap(final K[] k, final int[] v) {
	...
	public void forEach(final ObjectIntBiConsumer<? super K> consumer) {
		...
		final K key[] = ObjectIntOpenHashMap.this.key;
	...
	private abstract class MapIterator<ConsumerType> {
		...
		boolean mustReturnNullKey = ObjectIntOpenHashMap.this.containsNullKey;
		/**
		...
		private void shiftKeys(int pos) {
			...
			K curr;
			final K[] key = ObjectIntOpenHashMap.this.key;
			final int value[] = ObjectIntOpenHashMap.this.value;
			for (;;) {
				pos = ((last = pos) + 1) & mask;
				for (;;) {
					if (((curr = key[pos]) == null)) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				ObjectIntOpenHashMap.this.removeInt(wrapped.set(-pos - 1, null));
	...
	private final class EntryIterator extends MapIterator<Consumer<? super ObjectIntMap.Entry<K>>> implements Iterator<Entry<K>> {
	...
	private final class MapEntrySet extends AbstractSet<Entry<K>> implements FastEntrySet<K> {
		...
		public boolean contains(final Object o) {
			...
			if (((k) == null)) return ObjectIntOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			K curr;
			final K[] key = ObjectIntOpenHashMap.this.key;
			int pos;
			// The starting point.
		...
		@Override
		public void clear() {
			ObjectIntOpenHashMap.this.clear();
		}
	...
	private final class KeySet extends AbstractSet<K> {
		...
		public boolean remove(Object k) {
			...
			ObjectIntOpenHashMap.this.removeInt(k);
			return size != oldSize;
		}
	...
	@Override
	@SuppressWarnings("unchecked")
	public ObjectIntOpenHashMap<K> clone() {
		ObjectIntOpenHashMap<K> c;
		try {
			c = (ObjectIntOpenHashMap<K>)super.clone();

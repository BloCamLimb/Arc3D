 ...
 * @see Map
 */
public interface IntObjectMap<V> extends java.util.function.IntFunction<V> {
	/**
	...
	interface FastEntrySet<V> extends Set<Entry<V>> {
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super IntObjectMap.Entry<V>> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @see Map#entrySet()
	 */
	Set<IntObjectMap.Entry<V>> int2ObjectEntrySet();
	 ...
	 */
	default void forEach(final IntObjectBiConsumer<? super V> consumer) {
		final Set<IntObjectMap.Entry<V>> entrySet = int2ObjectEntrySet();
		final Consumer<IntObjectMap.Entry<V>> wrappingConsumer = (entry) -> consumer.accept(entry.getIntKey(), entry.getValue());
    ...
    default void putAll(IntObjectMap<? extends V> m) {
        Iterator<IntObjectMap.Entry<V>> i;
        if (m.int2ObjectEntrySet() instanceof IntObjectMap.FastEntrySet fastEntrySet) {
            i = fastEntrySet.fastIterator();
        } else {
            i = ((IntObjectMap<V>) m).int2ObjectEntrySet().iterator();
        }
        while (i.hasNext()) {
            final IntObjectMap.Entry<? extends V> e = i.next();
	 ...
	 * @since 8.0.0
	 */
	default V computeIfAbsent(final int key, final IntObjectMap<? extends V> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final V v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;

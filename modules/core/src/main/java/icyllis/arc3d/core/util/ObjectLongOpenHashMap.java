 ...
 * @see HashCommon
 */
public class ObjectLongOpenHashMap<K> implements ObjectLongMap<K>, java.io.Serializable, Cloneable, Hash {
	...
	public ObjectLongOpenHashMap(final int expected, final float f) {
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public ObjectLongOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public ObjectLongOpenHashMap(final ObjectLongMap<K> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public ObjectLongOpenHashMap(final ObjectLongMap<K> m) {
	 ...
	 */
	public ObjectLongOpenHashMap(final K[] k, final long[] v, final float f) {
		this(k.length, f);
	 ...
	 * @throws IllegalArgumentException if {@code k} and {@code v} have different lengths.
	 */
	public ObjectLongOpenHashMap(final K[] k, final long[] v) {
	...
	@Override
	public void putAll(ObjectLongMap<? extends K> m) {
	...
	private int find(final K k) {
		...
		int pos;
		// The starting point.
		if (((curr = key[pos = (icyllis.arc3d.core.util.HashCommon.mix((k).hashCode())) & mask]) == null)) return -(pos + 1);
	...
	public long removeLong(final Object k) {
		...
		int pos;
		// The starting point.
		if (((curr = key[pos = (icyllis.arc3d.core.util.HashCommon.mix((k).hashCode())) & mask]) == null)) return defRetValue;
	...
	public long getOrDefault(final Object k, final long defaultValue) {
		...
		int pos;
		// The starting point.
		if (((curr = key[pos = (icyllis.arc3d.core.util.HashCommon.mix((k).hashCode())) & mask]) == null)) return defaultValue;
		if (((k).equals(curr))) return value[pos];
		// There's always an unused entry.
		while (true) {
	...
	/** {@inheritDoc} */
	@Override
	public long computeIfAbsent(final K k, final java.util.function.ToLongFunction<? super K> mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public long computeIfAbsent(final K key, final ObjectLongMap<? super K> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
	...
	/** {@inheritDoc} */
	@Override
	public long merge(final K k, final long v, final java.util.function.BiFunction<? super Long, ? super Long, ? extends Long> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
	...
	/** {@inheritDoc} */
	@Override
	public void forEach(final ObjectLongBiConsumer<? super K> consumer) {
		if (containsNullKey) {
			consumer.accept(key[n], value[n]);
		}
		final K key[] = ObjectLongOpenHashMap.this.key;
	 ...
	 */
	final class MapEntry implements ObjectLongMap.Entry<K> {
		...
		@Override
		public int hashCode() {
			return ((key[index]) == null ? 0 : (key[index]).hashCode()) ^ icyllis.arc3d.core.util.HashCommon.long2int(value[index]);
		}
	...
	private abstract class MapIterator<ConsumerType> {
		...
		boolean mustReturnNullKey = ObjectLongOpenHashMap.this.containsNullKey;
		/**
		...
		public int nextEntry() {
			...
			for (;;) {
				if (--pos < 0) {
					...
					int p = (icyllis.arc3d.core.util.HashCommon.mix((k).hashCode())) & mask;
		...
		private void shiftKeys(int pos) {
			...
			K curr;
			final K[] key = ObjectLongOpenHashMap.this.key;
			final long value[] = ObjectLongOpenHashMap.this.value;
			for (;;) {
				pos = ((last = pos) + 1) & mask;
				for (;;) {
					if (((curr = key[pos]) == null)) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				ObjectLongOpenHashMap.this.removeLong(wrapped.set(-pos - 1, null));
	...
	private final class EntryIterator extends MapIterator<Consumer<? super ObjectLongMap.Entry<K>>> implements Iterator<Entry<K>> {
		...
		@Override
		final void acceptOnIndex(final Consumer<? super ObjectLongMap.Entry<K>> action, final int index) {
	...
	private final class FastEntryIterator extends MapIterator<Consumer<? super ObjectLongMap.Entry<K>>> implements Iterator<ObjectLongMap.Entry<K>> {
	...
	private final class MapEntrySet extends AbstractSet<Entry<K>> implements FastEntrySet<K> {
		@Override
		public Iterator<ObjectLongMap.Entry<K>> iterator() {
			return new EntryIterator();
		}

		@Override
		public Iterator<ObjectLongMap.Entry<K>> fastIterator() {
		...
		public boolean contains(final Object o) {
			...
			if (((k) == null)) return ObjectLongOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			K curr;
			final K[] key = ObjectLongOpenHashMap.this.key;
			int pos;
			// The starting point.
		...
		@Override
		public void clear() {
			ObjectLongOpenHashMap.this.clear();
		}

		/** {@inheritDoc} */
		@Override
		public void forEach(final Consumer<? super ObjectLongMap.Entry<K>> consumer) {
		...
		/** {@inheritDoc} */
		@Override
		public void fastForEach(final Consumer<? super ObjectLongMap.Entry<K>> consumer) {
	...
	private final class KeySet extends AbstractSet<K> {
		...
		public boolean remove(Object k) {
			...
			ObjectLongOpenHashMap.this.removeLong(k);
			return size != oldSize;
		}
	...
	@Override
	@SuppressWarnings("unchecked")
	public ObjectLongOpenHashMap<K> clone() {
		ObjectLongOpenHashMap<K> c;
		try {
			c = (ObjectLongOpenHashMap<K>)super.clone();
	...
	public int hashCode() {
		...
		for (int j = realSize(), i = 0, t = 0; j-- != 0;) {
			...
			t ^= icyllis.arc3d.core.util.HashCommon.long2int(value[i]);
			h += t;
			i++;
		}
		// Zero / null keys have hash zero.
		if (containsNullKey) h += icyllis.arc3d.core.util.HashCommon.long2int(value[n]);
		return h;
	}
	...
	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
		...
		for (int i = size, pos; i-- != 0;) {
			...
			if (((k) == null)) {
				...
				pos = (icyllis.arc3d.core.util.HashCommon.mix((k).hashCode())) & mask;

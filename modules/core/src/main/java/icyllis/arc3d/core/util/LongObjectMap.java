 ...
 * @see Map
 */
public interface LongObjectMap<V> extends LongFunction<V> {
	/**
	...
	interface FastEntrySet<V> extends Set<Entry<V>> {
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super LongObjectMap.Entry<V>> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @see Map#entrySet()
	 */
	Set<LongObjectMap.Entry<V>> long2ObjectEntrySet();
    ...
    boolean containsValue(final Object v);
	 ...
	 */
	default void forEach(final LongObjectBiConsumer<? super V> consumer) {
		final Set<LongObjectMap.Entry<V>> entrySet = long2ObjectEntrySet();
		final Consumer<LongObjectMap.Entry<V>> wrappingConsumer = (entry) -> consumer.accept(entry.getLongKey(), entry.getValue());
    ...
    default void putAll(LongObjectMap<? extends V> m) {
        Iterator<LongObjectMap.Entry<V>> i;
        if (m.long2ObjectEntrySet() instanceof LongObjectMap.FastEntrySet fastEntrySet) {
            i = fastEntrySet.fastIterator();
        } else {
            i = ((LongObjectMap<V>) m).long2ObjectEntrySet().iterator();
        }
        while (i.hasNext()) {
            final LongObjectMap.Entry<? extends V> e = i.next();
	 ...
	 * @since 8.0.0
	 */
	default V computeIfAbsent(final long key, final LongObjectMap<? extends V> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final V v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;

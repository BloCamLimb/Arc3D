 ...
 * @see HashCommon
 */
public class IntObjectOpenHashMap<V> implements IntObjectMap<V>, java.io.Serializable, Cloneable, Hash {
	 ...
	 * @param f the load factor.
	 */
	public IntObjectOpenHashMap(final IntObjectMap<V> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public IntObjectOpenHashMap(final IntObjectMap<V> m) {
	 ...
	 */
	public IntObjectOpenHashMap(final int[] k, final V[] v, final float f) {
		this(k.length, f);
	 ...
	 * @throws IllegalArgumentException if {@code k} and {@code v} have different lengths.
	 */
	public IntObjectOpenHashMap(final int[] k, final V[] v) {
	...
	/** {@inheritDoc} */
	@Override
	public V computeIfAbsent(final int key, final IntObjectMap<? extends V> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
	...
	/** {@inheritDoc} */
	@Override
	public void forEach(final IntObjectBiConsumer<? super V> consumer) {
		if (containsNullKey) {
			consumer.accept(key[n], value[n]);
		}
		final int key[] = IntObjectOpenHashMap.this.key;
	 ...
	 */
	final class MapEntry implements IntObjectMap.Entry<V> {
	...
	private abstract class MapIterator<ConsumerType> {
		...
		boolean mustReturnNullKey = IntObjectOpenHashMap.this.containsNullKey;
		/**
		...
		private void shiftKeys(int pos) {
			...
			int curr;
			final int[] key = IntObjectOpenHashMap.this.key;
			final V value[] = IntObjectOpenHashMap.this.value;
			for (;;) {
				pos = ((last = pos) + 1) & mask;
				for (;;) {
					if (((curr = key[pos]) == (0))) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				IntObjectOpenHashMap.this.remove(wrapped.getInt(-pos - 1));
	...
	private final class EntryIterator extends MapIterator<Consumer<? super IntObjectMap.Entry<V>>> implements Iterator<Entry<V>> {
		...
		@Override
		final void acceptOnIndex(final Consumer<? super IntObjectMap.Entry<V>> action, final int index) {
	...
	private final class FastEntryIterator extends MapIterator<Consumer<? super IntObjectMap.Entry<V>>> implements Iterator<IntObjectMap.Entry<V>> {
	...
	private final class MapEntrySet extends AbstractSet<Entry<V>> implements FastEntrySet<V> {
		...
		@Override
		public Iterator<IntObjectMap.Entry<V>> fastIterator() {
		...
		public boolean contains(final Object o) {
			...
			if (((k) == (0))) return IntObjectOpenHashMap.this.containsNullKey && java.util.Objects.equals(value[n], v);
			int curr;
			final int[] key = IntObjectOpenHashMap.this.key;
			int pos;
			// The starting point.
		...
		@Override
		public void clear() {
			IntObjectOpenHashMap.this.clear();
		}

		/** {@inheritDoc} */
		@Override
		public void forEach(final Consumer<? super IntObjectMap.Entry<V>> consumer) {
		...
		/** {@inheritDoc} */
		@Override
		public void fastForEach(final Consumer<? super IntObjectMap.Entry<V>> consumer) {
	...
	private final class KeySet extends AbstractIntSet {
		...
		public boolean remove(int k) {
			...
			IntObjectOpenHashMap.this.remove(k);
			return size != oldSize;
		}
	...
	@Override
	@SuppressWarnings("unchecked")
	public IntObjectOpenHashMap<V> clone() {
		IntObjectOpenHashMap<V> c;
		try {
			c = (IntObjectOpenHashMap<V>)super.clone();

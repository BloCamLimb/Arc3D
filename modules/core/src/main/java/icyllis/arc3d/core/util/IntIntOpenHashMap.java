 ...
 * @see HashCommon
 */
public class IntIntOpenHashMap implements IntIntMap, java.io.Serializable, Cloneable, Hash {
	...
	public IntIntOpenHashMap(final int expected, final float f) {
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public IntIntOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public IntIntOpenHashMap(final IntIntMap m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public IntIntOpenHashMap(final IntIntMap m) {
	 ...
	 */
	public IntIntOpenHashMap(final int[] k, final int[] v, final float f) {
		this(k.length, f);
	 ...
	 * @throws IllegalArgumentException if {@code k} and {@code v} have different lengths.
	 */
	public IntIntOpenHashMap(final int[] k, final int[] v) {
	...
	private int find(final int k) {
		...
		int pos;
		// The starting point.
		if (((curr = key[pos = (icyllis.arc3d.core.util.HashCommon.mix((k))) & mask]) == (0))) return -(pos + 1);
	...
	public int addTo(final int k, final int incr) {
		...
		if (((k) == (0))) {
			...
			if (!((curr = key[pos = (icyllis.arc3d.core.util.HashCommon.mix((k))) & mask]) == (0))) {
				...
				while (!((curr = key[pos = (pos + 1) & mask]) == (0))) if (((curr) == (k))) return addToValue(pos, incr);
			}
		}
		key[pos] = k;
	...
	public int remove(final int k) {
		...
		int pos;
		// The starting point.
		if (((curr = key[pos = (icyllis.arc3d.core.util.HashCommon.mix((k))) & mask]) == (0))) return defRetValue;
	...
	public int getOrDefault(final int k, final int defaultValue) {
		...
		int pos;
		// The starting point.
		if (((curr = key[pos = (icyllis.arc3d.core.util.HashCommon.mix((k))) & mask]) == (0))) return defaultValue;
		if (((k) == (curr))) return value[pos];
		// There's always an unused entry.
		while (true) {
	...
	/** {@inheritDoc} */
	@Override
	public int computeIfAbsent(final int key, final IntIntMap mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
	...
	/** {@inheritDoc} */
	@Override
	public int computeIfAbsentNullable(final int k, final java.util.function.IntFunction<? extends Integer> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final int pos = find(k);
		if (pos >= 0) return value[pos];
	...
	/** {@inheritDoc} */
	@Override
	public int computeIfPresent(final int k, final java.util.function.BiFunction<? super Integer, ? super Integer, ? extends Integer> remappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public int compute(final int k, final java.util.function.BiFunction<? super Integer, ? super Integer, ? extends Integer> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Integer newValue = remappingFunction.apply(Integer.valueOf(k), pos >= 0 ? Integer.valueOf(value[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if (((k) == (0))) removeNullEntry();
				else removeEntry(pos);
	...
	/** {@inheritDoc} */
	@Override
	public int mergeInt(final int k, final int v, java.util.function.IntBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
	...
	/** {@inheritDoc} */
	@Override
	public int merge(final int k, final int v, final java.util.function.BiFunction<? super Integer, ? super Integer, ? extends Integer> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
	...
	/** {@inheritDoc} */
	@Override
	public void forEach(final IntIntBiConsumer consumer) {
		if (containsNullKey) {
			consumer.accept(key[n], value[n]);
		}
		final int key[] = IntIntOpenHashMap.this.key;
	 ...
	 */
	final class MapEntry implements IntIntMap.Entry {
	...
	private abstract class MapIterator<ConsumerType> {
		...
		boolean mustReturnNullKey = IntIntOpenHashMap.this.containsNullKey;
		/**
		...
		public int nextEntry() {
			...
			for (;;) {
				if (--pos < 0) {
					...
					int p = (icyllis.arc3d.core.util.HashCommon.mix((k))) & mask;
		...
		private void shiftKeys(int pos) {
			...
			int curr;
			final int[] key = IntIntOpenHashMap.this.key;
			final int value[] = IntIntOpenHashMap.this.value;
			for (;;) {
				pos = ((last = pos) + 1) & mask;
				for (;;) {
					if (((curr = key[pos]) == (0))) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				IntIntOpenHashMap.this.remove(wrapped.getInt(-pos - 1));
	...
	private final class EntryIterator extends MapIterator<Consumer<? super IntIntMap.Entry>> implements Iterator<Entry> {
		...
		@Override
		final void acceptOnIndex(final Consumer<? super IntIntMap.Entry> action, final int index) {
	...
	private final class FastEntryIterator extends MapIterator<Consumer<? super IntIntMap.Entry>> implements Iterator<IntIntMap.Entry> {
	...
	private final class MapEntrySet extends AbstractSet<Entry> implements FastEntrySet {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if (((k) == (0))) return IntIntOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			int curr;
			final int[] key = IntIntOpenHashMap.this.key;
			int pos;
			// The starting point.
		...
		@Override
		public void clear() {
			IntIntOpenHashMap.this.clear();
		}

		/** {@inheritDoc} */
		@Override
		public void forEach(final Consumer<? super IntIntMap.Entry> consumer) {
			if (containsNullKey) consumer.accept(new MapEntry(n));
			final int key[] = IntIntOpenHashMap.this.key;
			for (int pos = n; pos-- != 0;) if (!((key[pos]) == (0))) consumer.accept(new MapEntry(pos));
		}

		/** {@inheritDoc} */
		@Override
		public void fastForEach(final Consumer<? super IntIntMap.Entry> consumer) {
	...
	private final class KeySet extends AbstractIntSet {
		...
		public boolean remove(int k) {
			...
			IntIntOpenHashMap.this.remove(k);
			return size != oldSize;
		}
	...
	public IntIntOpenHashMap clone() {
		IntIntOpenHashMap c;
		try {
			c = (IntIntOpenHashMap)super.clone();
	...
	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
		...
		for (int i = size, pos; i-- != 0;) {
			...
			if (((k) == (0))) {
				...
				pos = (icyllis.arc3d.core.util.HashCommon.mix((k))) & mask;

 ...
 */
public class ByteArrayList implements Cloneable, java.io.Serializable {
	 ...
	 */
	public ByteArrayList(final ByteArrayList l) {
	...
	private void grow(int capacity) {
		...
		if (a != ByteArrays.DEFAULT_EMPTY_ARRAY) capacity = (int)Math.max(Math.min((long)a.length + (a.length >> 1), icyllis.arc3d.core.util.Arrays.MAX_ARRAY_SIZE), capacity);
	...
	public boolean addAll(final int index, final ByteArrayList l) {
		ensureIndex(index);
		final int n = l.size();
		if (n == 0) return false;
		grow(size + n);
